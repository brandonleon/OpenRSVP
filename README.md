...
An HTTP-based MCP (Model Context Protocol) server is available at `~/Documents/Development/openrsvp-mcp-server` for programmatic event management via Claude Code or other LLM tools.

### Quick Start

```bash
cd ~/Documents/Development/openrsvp-mcp-server
uv pip install -e .
```
...
mcp_servers:
  openrsvp:
    command: /home/brandonleon/.hermes/hermes-agent/venv/bin/openrsvp-mcp
    env:
      OPENRSVP_URL: https://openrsvp.kaotic.cc
      MCP_HOST: 127.0.0.1
      MCP_PORT: "3000"
    enabled: true
```
...
- `list_events` — List public events with pagination & date filtering
- `get_event` — Get event details by ID
- `create_event` — Create a new event (no auth required; returns admin_token)
...
See `~/Documents/Development/openrsvp-mcp-server/README.md` for full documentation.
